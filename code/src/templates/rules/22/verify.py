...
def verify(victim_apk : str, service_name : str):
    ...
    try:
        proc = subp.run(["adb", "logcat", "ActivityManager:V", "*:S"], stdout=subp.PIPE, stderr=subp.PIPE, timeout=20, text=True)
        logs = proc.stdout
    ...
    if isinstance(logs, bytes):
        logs = logs.decode()
    parse_logs(logs)
    target,ok = find_broadcast_target(victim_apk, service_name, logs)
    return 0 if ok else 1

def find_broadcast_target(victim_apk, service_name, out):
    target = ""
    print("Looking for Start proc .* for broadcast {{{}/{}}}".format(victim_apk, service_name))
    regex = r"Start proc .* for broadcast {" + re.escape(victim_apk) + r'\/' + re.escape(service_name) + r"}"
...
def parse_args():
    ...
    parser.add_argument("receiver", help="the targeted receiver name")
...
def main(args):
    ...
    return verify(victimApk.get_package(), args.receiver)
