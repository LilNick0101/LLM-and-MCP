...
def capture_traffic(duration_seconds=60, output_file="traffic.mitm"):
    ...
    process = subp.Popen(
        ["mitmdump","-s", "mitm_filter.py", "-w", output_file, "-p", PORT, "--flow-detail", str(VERBOSITY)]
    )
...
def extract_flows(flow_file):
    ...
    # We call mitmdump again to read and format the saved file
    try:
...
def parse_args():
    ...
    parser.add_argument("-avd", help="the name of the avd to use (default: {})".format(ANDROID_AVD), default=ANDROID_AVD)
